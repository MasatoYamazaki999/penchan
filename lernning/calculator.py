...
class CaluGui(object):
    def __init__(self, app=None):
        ...
        app.geometry('300x300') # Window のサイズ

        # Frame Setting
        calc_frame = ttk.Frame(app, width=100, height=30) # 計算式と結果用のFrame
        calc_frame.propagate(False) # サイズが固定される
        calc_frame.pack(side=tk.TOP, padx=5, pady=5) # 余白の設定
        button_frame = ttk.Frame(app, width=100, height=100) # 計算ボタン用のFrame
        ...
        for y, row in enumerate(BUTTON, 1): # Buttonの配置
            for x, num in enumerate(row):
                button = tk.Button(button_frame, text=num, font=('', 15), width=2, height=1)
