...
class Client():
    def __init__(self):
        ...
        # 壁画像のロード
        Block.image = load_image("wall1.png", -1)
        # 自弾画像のロード
        Shot.image = load_image("fireball.png", -1)
        # 相手弾画像のロード
        EnemyShot.image = load_image("fireball.png", -1)
        # 下り階段画像のロード
        DownStaireway.image = load_image("wall2.png", -1)
        # 上り階段画像のロード
        UpStaireway.image = load_image("wall3.png", -1)
...
class Map:
    ...
    def draw(self):
        ...
        #                  (self.my_char.rect.x, self.my_char.rect.y + self.GS), 
        #                  (self.my_char.rect.x + 32, self.my_char.rect.y + self.GS), 2)
    ...
    def load(self, filename):
        """マップをロードしてスプライトを作成"""
        map = []
        fp = open(os.path.join("roaming-ralph/" + filename), "r")
        for line in fp:
...
def load_image(filename, colorkey=None):
    """画像をロードして画像と矩形を返す"""
    filename = os.path.join("roaming-ralph/data", filename)
    # print(Path.cwd())
    try:
